...
// Query to select all data from the 'personnel' table
$query = "SELECT personnel.*, user.firstname, user.lastname, user.role 
          FROM personnel 
                ...
                <tbody class="v-2 text-light">
                    ...
                    if ($personnel) {
                        foreach ($personnel as $row) {
                            ...
                            echo '<td>' . (isset($row['computer_skills']) ? $row['computer_skills'] : 'N/A') . '</td>';
                            echo '<td>' . (isset($row['availability']) ? $row['availability'] : 'N/A') . '</td>';
                            echo '<td>' . (isset($row['certification']) ? $row['certification'] : 'N/A') . '</td>';
                            echo '<td>';
                            if (isset($row['user_id']) && isset($row['personnel_id'])) {
                                echo '<a href="tfixshopowner_reqservice3.php?user_id=' . (int)$row['user_id'] . '&personnel_id=' . (int)$row['personnel_id'] . '" class="btn btn-primary">View Details</a>';
                            } else {
