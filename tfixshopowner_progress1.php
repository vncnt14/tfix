...
$query = "SELECT ar.*, rf.user_id, u.firstname, u.lastname, ar.unit_brand
          FROM assignref ar
                    ...
                    if ($result) {
                        foreach ($result as $progressData) {
                            ...
                            if ($progressData['unit_status'] != 'Waiting for payment') {
                                ...
                                echo '<td><a href="tfixshopowner_payment.php?user_id=' . $progressData['user_id'] . '&reqform_id=' . $progressData['reqform_id'] . '&unit_brand=' . $progressData['unit_brand']. '&parts_name=' . $progressData['parts_name'].'" class="btn btn-primary">View</a></td>';
                            } else {
