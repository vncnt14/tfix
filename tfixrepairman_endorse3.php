...
$repairman_id = $_SESSION['repairman_id'];
                    ...
                    if ($result) {
                        foreach ($result as $row) {
                           ...
                           echo '<td>' . (isset($row['unit_status']) ? $row['unit_status'] : 'N/A') . '</td>';
                            echo '<td>';
                             if ($row['unit_status'] != 'Finished') {
                                // If not completed, render the clickable "View" button
                                echo '<td><a href="tfixrepairman_service1.php?user_id=' . $row['user_id'] . '&reqform_id=' . $row['reqform_id'] . '" class="btn btn-primary">View</a></td>';
                            } else {
