...
if (isset($_GET['user_id']) && isset($_GET['reqform_id'])) {
    ...
    $reqformData = mysqli_query($connection, "SELECT * FROM reqform WHERE user_id = '$user_id' AND reqform_id = '$reqform_id' AND is_deleted = '0'");
    ...
    $result = mysqli_query($connection, "SELECT user.user_id, user.firstname, user.lastname, reqform_id, date_requested, unit_type, unit_brand
    FROM user
    JOIN reqform ON reqform.user_id = user.user_id WHERE reqform.is_deleted = '0'");
...
<body>
                    ...
                    if ($reqformData) {
                        foreach ($result as $row) {
                            ...
                            if (isset($row['user_id']) && isset($row['reqform_id'])) {
                                echo '<button class="btn btn-primary" onclick="redirectToDiagnose(' . (int)$row['user_id'] . ', ' . (int)$row['reqform_id'] . ', \'' . strtolower($row['unit_type']) . '\')">Diagnose</button>';
                            } else {
    ...
    <script>
        function redirectToDiagnose(userId, reqformId, unitType) {
            if (unitType === 'refrigerator') {
                window.location.href = "tfixpersonnel_progress.php?user_id=" + userId + "&reqform_id=" + reqformId;
            } else if (unitType === 'electricfan') {
                window.location.href = "tfixpersonnel_progress1.php?user_id=" + userId + "&reqform_id=" + reqformId;
            } else {
                console.error("Invalid unit_name: " + unitType);
            }
        }
    </script>
