...
$query = "SELECT ar.*, u.firstname, u.lastname, rf.date_requested
          FROM assignref ar
          JOIN user u ON ar.user_id = u.user_id
          JOIN reqform rf ON ar.reqform_id = rf.reqform_id WHERE rf.is_deleted= '0'";
                    ...
                    if ($result) {
                        foreach ($result as $row) {
                            ...
                            if (isset($row['user_id']) && isset($row['reqform_id'])) {
                                echo '<a href="tfixrepairman_endorseoutput.php?user_id=' . (int)$row['user_id'] . '&reqform_id=' . (int)$row['reqform_id'] . '" class="btn btn-primary">View Details</a>';
                            } else {
