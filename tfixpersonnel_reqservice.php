...
$query = "SELECT user.user_id, user.firstname, user.lastname, reqform_id, date_requested
          FROM user
          JOIN reqform ON reqform.user_id = user.user_id WHERE is_deleted = '0'";
                    ...
                    if ($result) {
                        foreach ($result as $row) {
                            ...
                            if (isset($row['user_id']) && isset($row['reqform_id'])) {
                                echo '<a href="tfixreqform_output2.php?user_id=' . (int)$row['user_id'] . '&reqform_id=' . (int)$row['reqform_id'] . '" class="btn btn-primary">View Details</a>';
                            } else {
